using Windows.ApplicationModel;
using Windows.ApplicationModel.Activation;
using Windows.Foundation;
using Windows.Foundation.Collections;
using Microsoft.UI.Xaml;
using Microsoft.UI.Xaml.Controls;
using Microsoft.UI.Xaml.Controls.Primitives;
using Microsoft.UI.Xaml.Data;
using Microsoft.UI.Xaml.Input;
using Microsoft.UI.Xaml.Media;
using Microsoft.UI.Xaml.Navigation;
using Microsoft.UI.Xaml.Shapes;
using Microsoft.Windows.AppNotifications;

// To learn more about WinUI, the WinUI project structure,
// and more about our project templates, see: http://aka.ms/winui-project-info.

namespace FocusTrace;

/// <summary>
/// Provides application-specific behavior to supplement the default Application class.
/// </summary>
public partial class App : Application
{
    public MainWindow? MainWindowHost { get; private set; }
    public FocusProfileData Profile { get; } = FocusDataStore.LoadProfile();
    
    /// <summary>
    /// Initializes the singleton application object.  This is the first line of authored code
    /// executed, and as such is the logical equivalent of main() or WinMain().
    /// </summary>
    public App()
    {
        InitializeComponent();
    }

    /// <summary>
    /// Invoked when the application is launched.
    /// </summary>
    /// <param name="args">Details about the launch request and process.</param>
    protected override void OnLaunched(Microsoft.UI.Xaml.LaunchActivatedEventArgs args)
    {
        FocusNotificationService.Enabled = Profile.SystemNotificationsEnabled;
        StartupRegistrationService.SetEnabled(Profile.LaunchAtLoginEnabled);
        bool hasPackageIdentity = HasPackageIdentity();
        if (hasPackageIdentity)
        {
            try
            {
                AppNotificationManager.Default.Register();
            }
            catch
            {
                // The app remains fully usable if Windows notifications are unavailable.
            }
        }

        MainWindowHost = new MainWindow();
        MainWindowHost.Activate();
        if (args.Arguments.Contains("--startup", StringComparison.OrdinalIgnoreCase) &&
            Profile.TrayModeEnabled)
        {
            MainWindowHost.HideToTray();
        }
    }

    private static bool HasPackageIdentity()
    {
        try
        {
            _ = Package.Current.Id.Name;
            return true;
        }
        catch
        {
            return false;
        }
    }

}
